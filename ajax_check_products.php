...
// 1. Exclude non-clothing items and other unwanted terms
$excludedTerms = [
    'Лекало паперове',
    'Листівка А5',
    'Послуга пошиття',
...
// 1.1 Exclude non-clothing categories (shoes, accessories, etc.)
$excludedCategories = [
    'АКСЕСУАРИ',
    'ВЗУТТЯ',
    'Біжутерія',
...
// 2. Exclude stock <= 0 (where all stocks are 0 or -1, or maybe where they are all <= 0)
// "забрати товар якого 0 на залишку чи -1" - usually means if qty_1c <= 0 and qty_site <= 0.
// Let's assume if any stock is > 0, we keep it. If all are <= 0, we remove it.
$where[] = "(COALESCE(qty_1c, 0) > 0 OR COALESCE(qty_site, 0) > 0 OR COALESCE(qty_keycrm, 0) > 0)";
    ...
    foreach ($_POST['columns'] as $i => $col) {
        if (isset($col['search']['value']) && $col['search']['value'] !== '') {
            ...
            if ($i == 2) {
                ...
                $where[] = "(name_1c LIKE ? OR name_site LIKE ? OR name_keycrm LIKE ?)";
                $params  = array_merge($params, ["%$val%", "%$val%", "%$val%"]);
            } elseif ($i == 6) {
                $where[]  = "category LIKE ?";
                $params[] = "%$val%";
            } elseif ($i == 7) {
